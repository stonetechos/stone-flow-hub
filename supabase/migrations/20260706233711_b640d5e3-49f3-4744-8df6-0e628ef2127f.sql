...
DECLARE
  ...
  vr_ids   uuid[] := ARRAY[]::uuid[];
  vq_ids   uuid[] := ARRAY[]::uuid[];
  quote_ids uuid[] := ARRAY[]::uuid[];
  so_ids   uuid[] := ARRAY[]::uuid[];
  po_ids   uuid[] := ARRAY[]::uuid[];
  inv_ids  uuid[] := ARRAY[]::uuid[];
  mo_ids   uuid[] := ARRAY[]::uuid[];
  ...
  cust_cities text[] := ARRAY[
    'Bengaluru','Mumbai','Ahmedabad','Chennai','Hyderabad','Pune','Delhi','Kolkata','Kochi',
    'Jaipur','Surat','Coimbatore','Indore','Nagpur','Vizag','Bhopal','Lucknow','Chandigarh','Goa','Mysuru'
  ];
  vend_names text[] := ARRAY[
    'Rajasthan Marble Works','Precision Waterjet Co','CNC Stone Studio',
    'Kishangarh Stone Traders','Makrana White Co','Udaipur Green Marbles',
    'Vadodara CNC Cutters','Bengaluru Stone Studio','Coimbatore Granite House',
    'Kadapa Black Exports','Tandur Yellow Suppliers','Chennai Coastal Marble',
    'Jaipur Sandstone Guild','Ambaji Marble Depot','Rajasthani Craft House',
    'Karnataka Granite Co','Delhi Slabs India','Hyderabad Stone Yard',
    'Andhra Granite Traders','Global Waterjet Solutions'
  ];
  vend_cities text[] := ARRAY[
    'Kishangarh','Vadodara','Bengaluru','Ambaji','Udaipur','Jaipur','Coimbatore','Chennai','Kadapa','Tandur'
  ];
  prod_names text[] := ARRAY[
    ...
    'Botticino Classico Marble','Emperador Dark Marble','Beige Onyx Backlit Panel',
    'Green Onyx Feature Wall','Steel Grey Granite Countertop','River White Granite',
    'Kashmir White Granite','Bianco Carrara Marble Panel','Nero Marquina Marble',
    'Travertine Silver Cladding','Volakas White Marble','Crema Marfil Marble',
    'Jaisalmer Yellow Sandstone','Kota Blue Limestone'
  ];
  stages text[] := ARRAY['new_lead','customer_quotation_sent','vendor_approved','production','delivered'];
  enq_stages text[] := ARRAY['new_lead','qualified','rfq_sent','vendor_quotation_received','customer_quotation_sent','order_won','order_lost'];
  invoice_statuses public.invoice_status[] := ARRAY['draft','sent','partially_paid','paid','overdue']::public.invoice_status[];
  so_statuses public.sales_order_status[] := ARRAY['draft','confirmed','in_production','ready_to_dispatch','dispatched','completed']::public.sales_order_status[];
  ...
  v_amount numeric; v_paid numeric; v_days int;
BEGIN
  --------------------------------------------------------------------
  -- 1. Customers (~50)
  --------------------------------------------------------------------
  FOR i IN 1 .. array_length(cust_names,1) LOOP
    ...
    VALUES (
      cust_names[i],
      lower(replace(cust_names[i],' ','')) || '@demo.example',
      '+91-98' || lpad((10000000 + i)::text, 8, '0'),
      cust_cities[1 + (i % array_length(cust_cities,1))],
      'India','India', true, true,
      CASE WHEN i % 5 = 0 THEN 'temple'
           WHEN i % 4 = 0 THEN 'hospitality'
           WHEN i % 3 = 0 THEN 'commercial'
  ...
  FOR i IN 1 .. array_length(vend_names,1) LOOP
    ...
    VALUES (
      vend_names[i],
      vend_cities[1 + (i % array_length(vend_cities,1))],
      'Rajasthan','India', true,
  ...
  FOR i IN 1 .. array_length(prod_names,1) LOOP
    ...
    VALUES (
      prod_names[i],
      'Premium demo product — ' || prod_names[i],
      CASE WHEN i % 3 = 0 THEN 'piece' ELSE 'sqft' END,
      true
  ...
  FOR i IN 1 .. 60 LOOP
    ...
    VALUES (
      (SELECT name FROM public.customers WHERE id = v_cust) || ' – ' ||
        ...
        ' Phase ' || (1 + (i % 3)),
      v_cust,
      (SELECT city FROM public.customers WHERE id = v_cust),
  ...
  FOR i IN 1 .. 100 LOOP
    ...
    VALUES (
      v_proj, v_cust,
      enq_stages[1 + (i % array_length(enq_stages,1))]::public.lead_stage,
      ...
      (100000 + (i*12345) % 4000000),
      true
    ) RETURNING id INTO v_enq;
    enq_ids := array_append(enq_ids, v_enq);

    -- Backdate created_at across the last 24 months
    UPDATE public.enquiries
       SET created_at = now() - ((random()*720)::int || ' days')::interval,
           updated_at = now() - ((random()*30)::int || ' days')::interval
     WHERE id = v_enq;

    -- 1-3 items per enquiry
  ...
  --------------------------------------------------------------------
  -- 6. RFQs (~40) with vendor_requests + vendor_quotes
  --------------------------------------------------------------------
  FOR i IN 1 .. 40 LOOP
    ...
    VALUES (v_enq, v_proj,
            ...
            (ARRAY['draft','sent','responses_received','closed'])[1 + (i % 4)]::public.rfq_status,
            'Demo RFQ round ' || i, true)
    RETURNING id INTO v_rfq;
    rfq_ids := array_append(rfq_ids, v_rfq);

    -- Snapshot enquiry items into rfq_items
    ...
    FOR j IN 1 .. (2 + (i % 2)) LOOP
      ...
      VALUES (v_rfq, v_vend, 'pending'::public.vendor_response_status, now() - ((i*3) || ' days')::interval, true)
      RETURNING id INTO v_vr;
      vr_ids := array_append(vr_ids, v_vr);

      -- Half get a vendor quote back
      IF (i+j) % 2 = 0 THEN
  ...
  FOR i IN 1 .. 40 LOOP
    ...
    UPDATE public.quotes
       SET created_at = now() - ((random()*600)::int || ' days')::interval
     WHERE id = v_quote;
    ...
    FOR j IN 1 .. (2 + (i % 3)) LOOP
      ...
      VALUES (v_quote, v_prod,
              (SELECT name FROM public.products WHERE id = v_prod),
              80 + ((i*j*11) % 400), 'sqft',
              600 + ((i*j*29) % 1800), 18, j, true);
    END LOOP;
  END LOOP;
  ...
  FOR i IN 1 .. 30 LOOP
    ...
    VALUES (v_quote, v_proj, v_cust,
            so_statuses[1 + (i % array_length(so_statuses,1))],
            (CURRENT_DATE - ((i*11) || ' days')::interval)::date,
            (CURRENT_DATE + ((30 + i*3) || ' days')::interval)::date,
            true)
  ...
  FOR i IN 1 .. 25 LOOP
    ...
    VALUES (v_vend, v_proj,
            po_statuses[1 + (i % array_length(po_statuses,1))],
            (CURRENT_DATE - ((i*9) || ' days')::interval)::date,
            (CURRENT_DATE + ((i*3) || ' days')::interval)::date,
            'INR', true)
    RETURNING id INTO v_po;
    po_ids := array_append(po_ids, v_po);
  END LOOP;

  --------------------------------------------------------------------
  -- 10. Production orders (~25)
  --------------------------------------------------------------------
  FOR i IN 1 .. 25 LOOP
    ...
    VALUES (v_so, v_proj, v_cust, v_prod,
            100 + (i * 7), 'sqft',
    ...
    mo_ids := array_append(mo_ids, v_mo);
  END LOOP;

  --------------------------------------------------------------------
  -- 11. Invoices (~30) + items + payments
  --------------------------------------------------------------------
  FOR i IN 1 .. 30 LOOP
    v_quote := quote_ids[1 + (i % array_length(quote_ids,1))];
    v_proj := (SELECT project_id FROM public.quotes WHERE id = v_quote);
    ...
    inv_ids := array_append(inv_ids, v_inv);
    ...
    FOR j IN 1 .. (2 + (i % 3)) LOOP
      ...
      VALUES (v_inv, v_prod,
              (SELECT name FROM public.products WHERE id = v_prod),
              50 + ((i*j*7) % 300), 'sqft',
              700 + ((i*j*31) % 2000), 18, j, true);
    END LOOP;

    -- Payments (0-2 per invoice; triggers recalc totals & status)
    ...
    IF v_amount > 0 AND i % 3 <> 0 THEN
      ...
      VALUES (v_inv, v_paid,
              (ARRAY['upi','bank_transfer','cheque','cash','card'])[1 + (i % 5)]::public.payment_method,
              (CURRENT_DATE - ((i*11) || ' days')::interval)::date,
              true);
    END IF;
  END LOOP;
  ...
  --------------------------------------------------------------------
  -- 13. Followups (~80) — mix of past done + upcoming
  --------------------------------------------------------------------
  FOR i IN 1 .. 80 LOOP
    v_proj := proj_ids[1 + (i % array_length(proj_ids,1))];
    v_enq  := enq_ids[1 + (i % array_length(enq_ids,1))];
    ...
    VALUES (v_proj, v_enq,
            ...
            (ARRAY['call','whatsapp','email','visit'])[1 + (i % 4)]::public.followup_channel,
            true);
  END LOOP;
END;
$function$;
