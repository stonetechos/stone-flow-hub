...
AS $function$
DECLARE
  cust_ids uuid[] := ARRAY[]::uuid[]; vend_ids uuid[] := ARRAY[]::uuid[];
  prod_ids uuid[] := ARRAY[]::uuid[]; proj_ids uuid[] := ARRAY[]::uuid[];
  enq_ids uuid[] := ARRAY[]::uuid[]; quote_ids uuid[] := ARRAY[]::uuid[]; so_ids uuid[] := ARRAY[]::uuid[];
  ...
  v_rfq uuid; v_vr uuid; v_vq uuid; v_quote uuid; v_so uuid; v_inv uuid;
...
BEGIN
  FOR i IN 1 .. array_length(cust_names,1) LOOP
    ...
    RETURNING id INTO v_cust; cust_ids := array_append(cust_ids, v_cust);
  END LOOP;
  ...
  FOR i IN 1 .. array_length(vend_names,1) LOOP
    ...
    RETURNING id INTO v_vend; vend_ids := array_append(vend_ids, v_vend);
  END LOOP;
  ...
  FOR i IN 1 .. array_length(prod_names,1) LOOP
    ...
    RETURNING id INTO v_prod; prod_ids := array_append(prod_ids, v_prod);
  END LOOP;
  ...
  FOR i IN 1 .. 60 LOOP
    ...
    VALUES ((SELECT name FROM public.customers WHERE id = v_cust) || ' – ' ||
             (ARRAY['Lobby Cladding','Master Bath','Feature Wall','Temple Sanctum','Villa Facade','Reception Desk','Lift Lobby','Pool Deck','Kitchen Counter','Grand Staircase'])[1 + (i % 10)] || ' Phase ' || (1 + (i % 3)),
    ...
    RETURNING id INTO v_proj; proj_ids := array_append(proj_ids, v_proj);
  END LOOP;
  ...
  FOR i IN 1 .. 100 LOOP
    ...
    RETURNING id INTO v_enq; enq_ids := array_append(enq_ids, v_enq);
  ...
  FOR i IN 1 .. 40 LOOP
    ...
    FOR j IN 1 .. (2 + (i % 2)) LOOP
      ...
      IF (i+j) % 2 = 0 THEN
        ...
        INSERT INTO public.vendor_quote_items (vendor_quote_id, rfq_item_id, product_name_snapshot, price_per_unit, quantity, unit, is_demo)
        SELECT v_vq, ri.id, ri.product_name_snapshot, 400 + ((i*j*17) % 1200), ri.quantity, ri.unit, true
  ...
  FOR i IN 1 .. 40 LOOP
    ...
    VALUES (v_proj, v_cust,
            (ARRAY['draft','sent','accepted','rejected','converted'])[1 + (i % 5)]::public.quote_status, 'INR', true)
    RETURNING id INTO v_quote; quote_ids := array_append(quote_ids, v_quote);
  ...
  FOR i IN 1 .. 30 LOOP
    ...
    RETURNING id INTO v_so; so_ids := array_append(so_ids, v_so);
  END LOOP;
  ...
  FOR i IN 1 .. 30 LOOP
    ...
    VALUES (v_quote, v_proj, v_cust,
            ...
            (CURRENT_DATE - ((i*13) || ' days')::interval)::date + INTERVAL '15 days', 'INR', true)
  ...
  FOR i IN 1 .. 80 LOOP
    ...
    VALUES (v_proj, v_enq,
            ...
            (ARRAY['call','whatsapp','email','meeting'])[1 + (i % 4)]::public.followup_channel, true);
  END LOOP;
END;
$function$;
