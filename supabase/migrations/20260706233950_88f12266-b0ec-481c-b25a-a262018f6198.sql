...
BEGIN
  ...
  FOR i IN 1 .. 60 LOOP
    ...
    VALUES (
      ...
      proj_stages[1 + (i % array_length(proj_stages,1))]::public.lead_stage,
      true, true,
  ...
  FOR i IN 1 .. 40 LOOP
    ...
    FOR j IN 1 .. (2 + (i % 2)) LOOP
      ...
      VALUES (v_rfq, v_vend, 'pending'::public.vendor_request_status,
              now() - ((i*3) || ' days')::interval, true)
  ...
  FOR i IN 1 .. 30 LOOP
    ...
    IF v_amount > 0 AND i % 3 <> 0 THEN
      ...
      VALUES (v_inv, v_paid,
              pay_methods[1 + (i % array_length(pay_methods,1))],
              (CURRENT_DATE - ((i*11) || ' days')::interval)::date,
              true);
    END IF;
  END LOOP;
