...
BEGIN
  ...
  SELECT id INTO v1 FROM public.vendors WHERE company_name='Rajasthan Marble Works' AND is_demo ORDER BY created_at DESC LIMIT 1;
  SELECT id INTO v2 FROM public.vendors WHERE company_name='Precision Waterjet Co' AND is_demo ORDER BY created_at DESC LIMIT 1;
  SELECT id INTO v3 FROM public.vendors WHERE company_name='CNC Stone Studio' AND is_demo ORDER BY created_at DESC LIMIT 1;
  ...
  INSERT INTO public.products (name, description, default_unit, is_demo) VALUES
    ...
    ('Waterjet Medallion – Peacock','Multi-stone waterjet medallion 1200mm dia','nos',true),
    ('Temple Pillar Carving – Sandstone','Hand-carved traditional pillar 8ft','nos',true);
  ...
  SELECT id INTO p3 FROM public.products WHERE name LIKE 'Waterjet Medallion%' AND is_demo ORDER BY created_at DESC LIMIT 1;
  ...
  INSERT INTO public.projects (name, customer_id, city, stage, is_active, is_demo) VALUES
    ('Prestige Lakeside Tower – Lobby Cladding', c1, 'Bengaluru','in_progress',true,true),
    ('SP Vertex Villa – Master Bath', c2, 'Mumbai','in_progress',true,true),
    ('Adani Corporate HQ – Feature Wall', c3, 'Ahmedabad','planning',true,true),
    ('Sri Balaji Temple – Sanctum Restoration', c5, 'Tirupati','in_progress',true,true);
  ...
  INSERT INTO public.enquiries (project_id, customer_id, stage, requirement, is_demo) VALUES
    (pr1, c1, 'quoted', 'Statuario cladding, 4,500 sqft', true),
    (pr2, c2, 'rfq_sent', 'Master bath vanity + wall, 220 sqft', true),
    (pr4, c5, 'new', 'Sanctum wall carvings, custom', true);
  ...
  INSERT INTO public.inventory_items (product_id, location, quantity_on_hand, unit, block_no, lot_no, is_demo) VALUES
    (p1, 'Warehouse A', 12, 'nos', 'BLK-001', 'LOT-A', true),
    (p2, 'Warehouse A', 45, 'nos', 'BLK-002', 'LOT-B', true),
    (p4, 'Yard B',       8, 'nos', 'BLK-003', 'LOT-T', true);
END; $$;
