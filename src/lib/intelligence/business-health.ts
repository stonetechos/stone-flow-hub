...
export async function getBusinessHealth(): Promise<BusinessHealth> {
  ...
  const sales: HealthMetric = { key: "sales", label: "Sales", score: clamp(salesConv * 0.6 + salesActivity * 0.4), note: `${converted}/${sent} quotes converted` };
  ...
  const completedInstalls = installs.filter((i) => i.status === "completed" || i.actual_end_date).length;
  const totalInstalls = installs.length;
  const csScore = totalInstalls === 0 ? 70 : (completedInstalls / totalInstalls) * 100 - Math.min(30, instOverdue * 4);
  const cs: HealthMetric = { key: "csat", label: "Customer Satisfaction", score: clamp(csScore), note: `${completedInstalls}/${totalInstalls} installs completed` };
  ...
  const inPrev30 = (iso: string | null) => iso && new Date(iso).getTime() > now - 60 * 86_400_000 && new Date(iso).getTime() <= now - 30 * 86_400_000;
  const rev30 = receipts.filter((r) => in30(r.received_at ?? r.created_at)).reduce((a, r) => a + Number(r.net_amount ?? 0), 0);
  const revPrev30 = receipts.filter((r) => inPrev30(r.received_at ?? r.created_at)).reduce((a, r) => a + Number(r.net_amount ?? 0), 0);
  const revenue: HealthMetric = {
    key: "revenue_trend", label: "Revenue Trend (30d)", score: clamp(50 + (revPrev30 === 0 ? 20 : ((rev30 - revPrev30) / revPrev30) * 50)),
  ...
  const conversion: HealthMetric = {
    key: "conversion_trend", label: "Conversion Trend (30d)", score: clamp(50 + (convPrev === 0 ? 20 : ((convThis - convPrev) / Math.max(1, convPrev)) * 50)),
