 ...
 * *deployment* misconfiguration, not a request-specific failure: every
 * caller sees the same thing regardless of who they are or what they asked
 * for, and retrying never helps until an operator fixes the deployment's
 * environment variables.
 *
 * Distinct from `getSupabaseConfigStatus()` (config-status.ts), which only
 * ever observes the *client*-side var pair (`VITE_SUPABASE_URL` /
 * `VITE_SUPABASE_PUBLISHABLE_KEY`) and gates the whole app before it ever
 * renders. This helper exists because the server-side pair
 ...
 * fine, and the gap only surfaces the first time a page calls a
 * server function that needs it. Parsing the already-thrown error's own
 * message is deliberate rather than adding a new server round trip: the
 * message already carries the exact missing-var list, generated by the
 * same code path this helper recognizes.
 *
 * Returns the missing variable names (e.g. `["SUPABASE_URL"]`) if `err`
 * matches that exact shape, else `null` — never throws.
 */
const MISSING_SUPABASE_ENV_ERROR_RE =
  /^Missing Supabase environment variable\(s\): (.+)\. Connect Supabase in Lovable Cloud\.$/;

export function parseMissingSupabaseEnvError(err: unknown): string[] | null {
  const message = err instanceof Error ? err.message : typeof err === "string" ? err : "";
  const match = MISSING_SUPABASE_ENV_ERROR_RE.exec(message);
