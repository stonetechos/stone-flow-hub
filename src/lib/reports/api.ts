...
export const REPORTS = {
  async sales(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        .from("invoices")
        .select(
          "invoice_no, issue_date, total, amount_paid, balance_due, status, customer:customer_id(name)",
        )
  ...
  async purchases(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        ...
        .select("po_no, order_date, status, vendor:vendor_id(company_name)")
    ...
    return {
      ...
      rows: rows.map((r) => ({
        ...r,
        vendor: (r.vendor as { company_name?: string } | null)?.company_name ?? "",
      })),
    };
  },
  async vendorPerformance(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        ...
        .select("*, vendor:vendor_id(company_name, vendor_code)")
  ...
  async production(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        ...
        .select(
          "mfg_no, status, planned_start, planned_end, quantity, unit, product:product_id(name), project:project_id(name)",
        )
  ...
  async followups(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        .from("followups")
        .select("id, scheduled_at, status, project:project_id(name)")
        .eq("status", "pending")
        .order("scheduled_at")
        .limit(2000) as never,
    );
  ...
  async outstanding(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        .from("invoices")
        .select(
          "invoice_no, issue_date, due_date, total, balance_due, status, customer:customer_id(name)",
        )
        .gt("balance_due", 0)
        .order("due_date")
        .limit(2000) as never,
  ...
  async projectProfitability(): Promise<Report> {
    const rows = await sel<Array<Record<string, unknown>>>(
      supabase
        .from("projects")
        .select("project_code, name, stage, expected_value_inr, customer:customer_id(name)")
        .order("created_at", { ascending: false })
        .limit(2000) as never,
    );
    return {
