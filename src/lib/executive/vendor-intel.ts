...
async function computeVendorScores(): Promise<VendorScore[]> {
  ...
  const nameMap = new Map<string, string>(
    (vendRes.data ?? []).map((v) => [v.id, (v as { company_name: string }).company_name] as const),
  );
  const outMap = new Map<string, number>();
  for (const r of (ledRes.data ?? []) as Array<{
    ...
    outMap.set(
      r.vendor_id,
      (outMap.get(r.vendor_id) ?? 0) + Number(r.debit ?? 0) - Number(r.credit ?? 0),
    );
  }
  type Perf = {
    vendor_id: string;
...
export async function getVendorIntel(): Promise<VendorIntel> {
  ...
  return {
    ...
    most_reliable: scores
      .filter((s) => s.orders_count >= 2)
      .sort((a, b) => b.reliability - a.reliability)
      .slice(0, 15),
    fastest: scores
      .filter((s) => s.avg_dispatch_days != null)
      .sort((a, b) => (a.avg_dispatch_days ?? 999) - (b.avg_dispatch_days ?? 999))
      .slice(0, 15),
    lowest_cost: [...scores]
      .sort(
        (a, b) =>
          a.purchase_value / Math.max(1, a.orders_count) -
          b.purchase_value / Math.max(1, b.orders_count),
      )
      .slice(0, 15),
    ...
    dependency: [...scores]
      ...
      .map((s) => ({
        ...
        share_pct: (s.purchase_value / totalSpend) * 100,
      })),
  };
}
