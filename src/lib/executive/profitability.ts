...
export async function getProjectProfitability(): Promise<ProjectProfitability[]> {
  const [projRes, estRes, quoteRes, invRes, vpRes, poRes] = await Promise.all([
    supabase.from("projects").select("id,name,customer_id,customers(name)").eq("is_active", true).limit(500),
    supabase.from("estimates").select("id,total,material_cost,manufacturing_cost,installation_cost,freight_cost,other_cost,enquiries(project_id)"),
  ...
  for (const r of [projRes, estRes, quoteRes, invRes, vpRes, poRes]) if (r.error) throw new AppError(mapDbError(r.error));

  type Est = { id: string; total: number; material_cost: number; manufacturing_cost: number; installation_cost: number; freight_cost: number; other_cost: number; enquiries?: { project_id?: string | null } | null };
  ...
  const quoteTotal = sumBy((quoteRes.data ?? []) as Array<{ project_id: string; total: number }>, (r) => Number(r.total ?? 0));
  const invTotal = sumBy((invRes.data ?? []) as Array<{ project_id: string; total: number }>, (r) => Number(r.total ?? 0));
  const vpTotal = new Map<string, number>();
  for (const r of (vpRes.data ?? []) as Array<{ amount: number; grns?: { project_id?: string | null } | null }>) {
    const pid = r.grns?.project_id;
    if (pid) vpTotal.set(pid, (vpTotal.get(pid) ?? 0) + Number(r.amount ?? 0));
  }

  const rows: ProjectProfitability[] = [];
  for (const p of (projRes.data ?? []) as Array<{ id: string; name: string; customers?: { name?: string } | null }>) {
    ...
    const net = sales - material - procurement - install - labour - transport - Number(est?.other_cost ?? 0);
    rows.push({
      project_id: p.id,
      project_name: p.name,
