...
export async function buildDocumentContext({
  ...
  entityId,
  paymentLinkUrl,
}: BuildOpts): Promise<BuiltContext> {
  ...
  switch (entityType) {
    case "estimate": {
      const { data: e } = await from("estimates")
        .select(
          "*, customer:customers(id,name,primary_email,primary_phone), project:projects(id,name), items:estimate_items(*)",
        )
        .eq("id", entityId)
        .maybeSingle();
      ...
      return {
        ...
        defaultTo: {
          email: e?.customer?.primary_email ?? undefined,
          phone: e?.customer?.primary_phone ?? undefined,
        },
        ctx: {
          CustomerName: e?.customer?.name ?? "",
          ProjectName: e?.project?.name ?? "",
          EstimateNo: e?.estimate_no ?? "",
          StoneType: String(it.stone_type ?? ""),
          SurfaceFinish: String(it.surface_finish ?? ""),
          EdgeFinish: String(it.edge_finish ?? ""),
          Material: String(it.description ?? ""),
          Area: String(it.quantity ?? ""),
          Uom: String(it.unit ?? "sqft"),
    ...
    case "quote": {
      const { data: q } = await from("quotes")
        .select(
          "*, customer:customers(id,name,primary_email,primary_phone), project:projects(id,name)",
        )
        .eq("id", entityId)
        .maybeSingle();
      return {
        ...
        defaultTo: {
          email: q?.customer?.primary_email ?? undefined,
          phone: q?.customer?.primary_phone ?? undefined,
        },
        ctx: {
    ...
    case "invoice": {
      ...
      return {
        ...
        defaultTo: {
          email: i?.customer?.primary_email ?? undefined,
          phone: i?.customer?.primary_phone ?? undefined,
        },
        ctx: {
          ...
          PaymentLink: link,
          Date: fmtDate(i?.issue_date ?? i?.created_at),
        },
      };
    }
    case "receipt": {
      ...
      return {
        ...
        projectId: null,
        defaultTo: {
          email: r?.customer?.primary_email ?? undefined,
          phone: r?.customer?.primary_phone ?? undefined,
        },
        ctx: {
    ...
    case "sales_order": {
      ...
      return {
        ...
        projectId: so?.project?.id ?? null,
        defaultTo: {
          email: so?.customer?.primary_email ?? undefined,
          phone: so?.customer?.primary_phone ?? undefined,
        },
        ctx: {
          CustomerName: so?.customer?.name ?? "",
          ProjectName: so?.project?.name ?? "",
          SoNo: so?.so_no ?? "",
    ...
    case "dispatch": {
      const { data: d } = await from("dispatches")
        .select(
          "*, sales_order:sales_orders(customer_id,project_id, customer:customers(id,name,primary_email,primary_phone), project:projects(id,name))",
        )
        .eq("id", entityId)
        .maybeSingle();
      ...
      return {
        ...
        defaultTo: {
          ...
          phone: cust?.primary_phone ?? undefined,
        },
        ctx: {
    ...
    case "purchase_order": {
      ...
      return {
        ...
        projectId: null,
        defaultTo: {
          email: po?.vendor?.primary_email ?? undefined,
          phone: po?.vendor?.primary_phone ?? undefined,
        },
        ctx: {
