...
async function getSettings(supabase: SupabaseClient) {
  ...
  for (const r of rows.data ?? [])
    map.set((r as { key: string }).key, (r as { value: unknown }).value);
  return {
 ...
 */
export async function runWhatsappConnectionTest(supabase: SupabaseClient): Promise<{
  ok: boolean;
  checks: {
  ...
  if (!token) {
    checks.access_token.detail = "Missing WHATSAPP_ACCESS_TOKEN secret";
  ...
  if (!c.phone_number_id) checks.phone_number_id.detail = "Not configured";
    ...
    const r = await fetch(
      `https://graph.facebook.com/${WA_GRAPH_VERSION}/${encodeURIComponent(c.phone_number_id)}?fields=display_phone_number,verified_name`,
    ...
    if (r.status === 401 || r.status === 403) {
      checks.access_token.ok = false;
      checks.access_token.detail = "Invalid access token";
    } else if (r.status === 404) {
      checks.phone_number_id.detail = "Invalid Phone Number ID";
    } else if (!r.ok) {
      checks.phone_number_id.detail = j.error?.message || `HTTP ${r.status}`;
    } else {
      checks.phone_number_id.ok = true;
      checks.phone_number_id.detail =
        `${j.verified_name ?? ""} ${j.display_phone_number ?? ""}`.trim();
      checks.messaging_endpoint.ok = true;
      checks.messaging_endpoint.detail = "Reachable";
    }
  }
  ...
  if (!c.business_account_id) checks.business_account_id.detail = "Not configured";
    ...
    const r = await fetch(
      `https://graph.facebook.com/${WA_GRAPH_VERSION}/${encodeURIComponent(c.business_account_id)}?fields=name,timezone_id`,
      { headers: { Authorization: `Bearer ${token}` } },
    );
    ...
    if (r.status === 404) checks.business_account_id.detail = "Invalid WABA ID";
      ...
      checks.business_account_id.ok = true;
      checks.business_account_id.detail = j.name ?? "OK";
    }
  }

  const ok =
    checks.access_token.ok &&
    checks.phone_number_id.ok &&
    checks.business_account_id.ok &&
    checks.messaging_endpoint.ok;
  return { ok, checks };
}
...
export async function sendTest(
  ...
  if (channel === "email") {
    ...
    if (!to)
      return {
        ok: false,
        error: "No recipient — set test_email in Communication settings or pass a to override.",
      };
  ...
  if (channel === "whatsapp") {
    ...
    if (!to)
      return {
        ok: false,
        error: "No recipient — set test_phone in Communication settings or pass a to override.",
      };
...
export async function dispatchQueueBatch(
  ...
  for (const m of list) {
    ...
    const redirectedTo =
      isTest && m.channel === "email"
        ? s.mode.test_email || m.to_address
        : isTest && m.channel === "whatsapp"
          ? s.mode.test_phone || m.to_address
          : m.to_address;
