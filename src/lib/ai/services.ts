...
import {
  suggestHsnGst,
  estimateCost as estimateCostFn,
  marketPrice,
  recognizeStoneImage,
...
export interface DrawingAnalysis {
  ...
  estimated_wastage_pct: number;
  suggested_thickness_mm: number;
  suggested_fixing_method: string;
  suggested_production_route: string;
  confidence: number;
  notes?: string;
}
...
export function heuristicHsn(family?: string): HsnSuggestion {
  ...
  if (f.includes("mosaic") || f.includes("panel") || f.includes("cladding"))
    return { hsn: "68022190", confidence: 0.6, reason: "Worked marble/stone articles" };
  if (f.includes("veneer"))
    return { hsn: "68029900", confidence: 0.6, reason: "Flexible / worked stone" };
  if (
    f.includes("artwork") ||
    f.includes("mural") ||
    f.includes("inlay") ||
    f.includes("sculpture")
  )
    return { hsn: "97030090", confidence: 0.5, reason: "Original sculptures / art" };
