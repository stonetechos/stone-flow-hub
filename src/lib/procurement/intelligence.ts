...
export function buildRecommendations(rows: QuoteComparisonRow[]): ProcurementRecommendation[] {
  ...
  const pack = (
    kind: RecommendationKind,
    r: QuoteComparisonRow | null | undefined,
    reason: string,
  ) => {
    if (!r) return;
    out.push({
  ...
  pack(
    "cheapest",
    cheapest,
    `Lowest total cost ₹${cheapest.totalCost.toLocaleString("en-IN")}` +
  ...
  const fastest = submitted
    ...
    .reduce<QuoteComparisonRow | null>(
      (a, b) =>
        !a || (b.quote?.dispatch_days ?? Infinity) < (a.quote?.dispatch_days ?? Infinity) ? b : a,
      null,
    );
  pack(
    "fastest",
    fastest,
    fastest?.quote?.dispatch_days != null
  ...
  pack(
    ...
    preferred,
    preferred
      ? `Marked as preferred vendor with performance score ${Math.round(preferred.perf?.score ?? 0)}.`
      : "",
  );
