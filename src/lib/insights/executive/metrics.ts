...
export function buildMetrics(insights: ProcessedInsight[], summary: ExecutiveSummary): ExecutiveMetric {
  const resolved = insights.reduce(
    (sum, insight) => sum + Math.max(0, insight.mergedFrom.length - 1) + insight.supportingContext.length,
    0,
  );

  const attentionRequired = summary.critical + summary.warning;
  const completionRatio = summary.total === 0 ? 1 : Number((summary.healthy / summary.total).toFixed(2));
