...
export async function getCustomerTimeline(
  ...
  push(async () => {
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      at: r.created_at ?? "",
      title: `Enquiry ${r.enquiry_no ?? ""}`,
      subtitle: r.requirement ?? null,
      status: r.stage ?? null,
      refNo: r.enquiry_no ?? null,
      href: `/enquiries/${r.id}`,
    }));
  });
  push(async () => {
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      at: r.created_at ?? "",
      title: `Estimate ${r.estimate_no ?? ""}`,
      subtitle: r.template ?? null,
      amount: r.total != null ? Number(r.total) : null,
      status: r.status ?? null,
      refNo: r.estimate_no ?? null,
      href: `/estimates/${r.id}`,
    }));
  });
  push(async () => {
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      kind: "quote" as const,
      at: r.created_at ?? "",
      title: `Quotation ${r.quote_no ?? ""}`,
      amount: r.total != null ? Number(r.total) : null,
      status: r.status ?? null,
      refNo: r.quote_no ?? null,
      href: `/quotes/${r.id}`,
    }));
  });
  push(async () => {
    const { data } = await from("sales_orders")
      .select("*")
  ...
  push(async () => {
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      amount: r.total != null ? Number(r.total) : null,
      status: r.status ?? null,
      refNo: r.invoice_no ?? null,
      subtitle:
        r.balance_due != null && Number(r.balance_due) > 0 ? `Balance ₹${r.balance_due}` : "Paid",
      href: `/invoices/${r.id}`,
    }));
  });
  push(async () => {
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      amount: r.net_amount != null ? Number(r.net_amount) : null,
      status: r.status ?? null,
      refNo: r.receipt_no ?? null,
      href: `/receipts/${r.id}`,
    }));
  });
  push(async () => {
    // payments only join via invoice.customer_id
    const { data } = await from("payments")
      .select("*, invoice:invoices!inner(customer_id)")
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      subtitle: r.method ?? null,
    }));
  });
  push(async () => {
    const { data } = await from("dispatches")
      .select("*, sales_order:sales_orders!inner(customer_id)")
      .eq("sales_order.customer_id", customerId)
      .limit(limit);
    return (data ?? []).map((r: Any) => ({
      id: `dsp-${r.id}`,
      kind: "dispatch" as const,
      at: r.dispatch_date ?? r.created_at ?? "",
      title: `Dispatch ${r.dispatch_no ?? ""}`,
      status: r.status ?? null,
      refNo: r.dispatch_no ?? null,
      href: `/dispatch/${r.id}`,
    }));
  });
  push(async () => {
    const { data } = await from("installations")
      .select("*")
      .eq("customer_id", customerId)
      .limit(limit);
    return (data ?? []).map((r: Any) => ({
      ...
      status: r.status ?? null,
      refNo: r.installation_no ?? null,
      href: `/installations/${r.id}`,
    }));
  });

  push(async () => {
    const { data } = await from("site_visits")
      .select("*, project:projects!inner(customer_id)")
  ...
  push(async () => {
    const { data } = await from("followups")
      .select("*")
    ...
    return (data ?? []).map((r: Any) => ({
      ...
      title: `Follow-up`,
      status: r.status ?? null,
      subtitle: r.channel ?? r.notes ?? null,
    }));
  });
  push(async () => {
    const { data } = await from("message_queue")
      .select("*")
      .eq("customer_id", customerId)
      .limit(limit);
    return (data ?? []).map((r: Any) => ({
