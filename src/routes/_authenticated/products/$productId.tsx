...
function ProductHub() {
  ...
  const q = useQuery({ queryKey: ["products", "byId", productId], queryFn: () => getProduct(productId) });
  ...
  return (
    <div>
      ...
      <PageHeader
        title={p.name}
        subtitle={<span className="flex items-center gap-2"><span className="font-mono text-xs">{p.product_code}</span><Badge variant="secondary" className="capitalize">{p.stone_type}</Badge></span>}
      />
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="sales" className="mt-4">
          <RelatedList title="Sales History (Invoiced lines)"
            queryKey={["hub", "product", productId, "sales"]}
            queryFn={async () => (await hub.productInvoiceItems(productId)).map((r) => ({ ...r, id: r.id }))}
            linkFor={(r) => r.invoice ? { to: "/invoices/$invoiceId", params: { invoiceId: r.invoice.id } } : null}
            columns={[
              { header: "Invoice", cell: (r) => <span className="font-mono text-xs">{r.invoice?.invoice_no ?? "—"}</span> },
              { header: "Qty", cell: (r) => `${r.quantity} ${r.unit}` },
              { header: "Line total", cell: (r) => formatInr(r.line_total) },
              { header: "Status", cell: (r) => <Badge variant="outline">{r.invoice?.status ?? "—"}</Badge> },
            ]}
          />
        </TabsContent>

        <TabsContent value="projects" className="mt-4">
          <RelatedList title="Projects Used In"
        ...
        <TabsContent value="quotes" className="mt-4">
          <RelatedList title="Quotations"
            ...
            linkFor={(r) => r.quote ? { to: "/quotes/$quoteId", params: { quoteId: r.quote.id } } : null}
            columns={[
              { header: "Quote", cell: (r) => <span className="font-mono text-xs">{r.quote?.quote_no ?? "—"}</span> },
              { header: "Qty", cell: (r) => `${r.quantity} ${r.unit}` },
              { header: "Line total", cell: (r) => formatInr(r.line_total) },
              { header: "Status", cell: (r) => <Badge variant="outline">{r.quote?.status ?? "—"}</Badge> },
            ]}
          />
        </TabsContent>
        ...
        <TabsContent value="specs" className="mt-4">
          <Card className="shadow-1"><CardHeader><CardTitle className="text-sm">Specifications</CardTitle></CardHeader>
            <CardContent>
              <InfoGrid items={[
                { label: "Stone type", value: p.stone_type },
                { label: "Finish", value: p.finish },
        ...
        <TabsContent value="notes" className="mt-4">
          <NotesPanel table="products" id={productId} value={p.description} invalidateKey={["products", "byId", productId]} column="description" title="Notes / Description" />
        </TabsContent>
