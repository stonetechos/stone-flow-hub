...
function EditPurchaseOrderPage() {
  ...
  useEffect(() => {
    if (query.data) {
      const r = query.data;
      setForm({
        vendor_id: r.vendor_id, project_id: r.project_id, status: r.status,
        order_date: r.order_date, expected_date: r.expected_date, notes: r.notes,
      });
    }
  }, [query.data]);
  ...
  return (
    <div>
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/purchase-orders/$id", params: { id } })}>Cancel</Button>
