...
import { PURCHASE_ORDER_STATUSES, type PurchaseOrderCreateInput } from "@/lib/purchase-orders/schema";
...
function NewPurchaseOrderPage() {
  ...
  const [form, setForm] = useState<PurchaseOrderCreateInput>({
    vendor_id: null, project_id: null, status: "draft",
    order_date: today(), expected_date: null, notes: null,
  });
  ...
  const mut = useMutation({
    ...
    onSuccess: (row) => { toast.success(`PO ${row.po_no} created`); nav({ to: "/purchase-orders/$id", params: { id: row.id } }); },
    ...
    <div>
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        ...
        <QuickForm.MoreDetails>
          ...
          <Field label="Status">
            <Select value={form.status} onValueChange={(v) => set("status", v as PurchaseOrderCreateInput["status"])}>
          ...
          <Field label="Expected date">
            <Input type="date" value={form.expected_date ?? ""} onChange={(e) => set("expected_date", e.target.value || null)} />
          </Field>
        </QuickForm.MoreDetails>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/purchase-orders" })}>Cancel</Button>
