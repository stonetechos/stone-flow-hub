...
function NewInventoryPage() {
  ...
  const [form, setForm] = useState<InventoryCreateInput>({
    product_id: null, location: null, unit: null,
    quantity_on_hand: 0, reorder_level: 0, notes: null,
  });
  ...
  const mut = useMutation({
    ...
    onSuccess: (row) => { toast.success(`Stock ${row.stock_code} created`); nav({ to: "/inventory/$id", params: { id: row.id } }); },
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        <QuickForm.QuickFill>
          ...
          <Field label="Location">
            <Input value={form.location ?? ""} onChange={(e) => set("location", e.target.value || null)} placeholder="Warehouse / Bay" />
          </Field>
        </QuickForm.QuickFill>

        <QuickForm.MoreDetails>
          <Field label="Unit">
            <Input value={form.unit ?? ""} onChange={(e) => set("unit", e.target.value || null)} placeholder="sqft, slab…" />
          </Field>
          <Field label="On hand">
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/inventory" })}>Cancel</Button>
