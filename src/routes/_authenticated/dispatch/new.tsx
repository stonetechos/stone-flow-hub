...
function NewDispatchPage() {
  ...
  const [form, setForm] = useState<DispatchCreateInput>({
    sales_order_id: null, status: "planned",
    dispatch_date: today(), carrier: null, tracking_no: null, notes: null,
  });
  ...
  const mut = useMutation({
    ...
    onSuccess: (row) => { toast.success(`Dispatch ${row.dispatch_no} created`); nav({ to: "/dispatch/$id", params: { id: row.id } }); },
  ...
  return (
    <div>
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/dispatch" })}>Cancel</Button>
