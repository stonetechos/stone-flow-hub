...
function VendorIntelPage() {
  const q = useQuery({
    queryKey: ["exec", "vendor-intel"],
    queryFn: getVendorIntel,
    staleTime: 60_000,
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Reliability, quality, cost and dependency across your vendor base."
      />
      ...
      <Card className="mt-4">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">Vendor dependency analysis</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <li key={v.name}>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground">
                      {formatInr(v.purchase_value)} · {v.share_pct.toFixed(1)}%
                    </span>
                  </div>
                  <div className="h-2 rounded bg-muted overflow-hidden">
                    <div
                      className="h-full bg-primary"
                      style={{ width: `${Math.min(100, v.share_pct)}%` }}
                    />
                  </div>
                </li>
              ))}
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Concentration &gt; 30% for a single vendor is a supply-chain risk — diversify sources or
            negotiate SLAs.
          </p>
        </CardContent>
      </Card>
...
function VList({
  ...
  title: string;
  rows: VendorScore[];
  col: "spend" | "reliability" | "dispatch" | "quality" | "risk";
  tone?: "warn";
}) {
  return (
    <Card>
...
function formatVendor(
  ...
  if (col === "dispatch")
    return r.avg_dispatch_days != null ? `${r.avg_dispatch_days.toFixed(1)}d` : "—";
