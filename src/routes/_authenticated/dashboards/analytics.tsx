...
import {
  ...
  getCustomerAging,
  getVendorAging,
  getRevenueByProductFamily,
  getRevenueByCustomer,
  defaultRange,
  type Grain,
  type Range,
} from "@/lib/executive/timeseries";
...
function AnalyticsDashboard() {
  ...
  const trends = useQuery({
    queryKey: ["exec", "trends", range, grain],
    queryFn: () => getTrends(range, grain),
    staleTime: 60_000,
  });
  const custAging = useQuery({
    queryKey: ["exec", "cust-aging"],
    queryFn: getCustomerAging,
    staleTime: 60_000,
  });
  const vendAging = useQuery({
    queryKey: ["exec", "vend-aging"],
    queryFn: getVendorAging,
    staleTime: 60_000,
  });
  const byFamily = useQuery({
    queryKey: ["exec", "by-family", range],
    queryFn: () => getRevenueByProductFamily(range),
    staleTime: 60_000,
  });
  const byCustomer = useQuery({
    queryKey: ["exec", "by-customer", range],
    queryFn: () => getRevenueByCustomer(range),
    staleTime: 60_000,
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Interactive charts across sales, collections, procurement, aging and mix."
      />
      ...
      <Card className="mb-4">
        <CardContent className="flex flex-wrap items-end gap-3 pt-4">
          ...
          <div className="ml-auto flex gap-1">
            <Button size="sm" variant="outline" onClick={() => setRange(defaultRange(30))}>
              30d
            </Button>
            <Button size="sm" variant="outline" onClick={() => setRange(defaultRange(90))}>
              90d
            </Button>
            <Button size="sm" variant="outline" onClick={() => setRange(defaultRange(365))}>
              1y
            </Button>
          </div>
        </CardContent>
      ...
      {trends.isLoading || !trends.data ? (
        ...
        <ErrorBlock message={toUserMessage(trends.error)} />
      ) : (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <TrendChart
            title="Procurement — POs created"
            data={trends.data.procurement}
            money={false}
          />
...
function AgingCard({
  ...
  title: string;
  q: ReturnType<typeof useQuery<Awaited<ReturnType<typeof getCustomerAging>>, Error>>;
}) {
  return (
    <Card>
      <CardHeader className="pb-2">
      ...
      <CardContent style={{ height: 260 }}>
        {q.isLoading || !q.data ? (
          ...
          <ErrorBlock message={toUserMessage(q.error)} />
        ) : (
          <ResponsiveContainer width="100%" height="100%">
...
function PieCard({
  ...
  title: string;
  q: ReturnType<typeof useQuery<Array<{ label: string; value: number }>, Error>>;
}) {
  return (
    <Card>
      <CardHeader className="pb-2">
