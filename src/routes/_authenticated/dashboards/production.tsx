    ...
    queryFn: async () => {
      ...
      return {
        ...
        overdue: orders.filter(
          (o) => o.planned_end && o.planned_end < today && o.status !== "completed",
        ).length,
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Live workshop status, QC and installation."
      />
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi
          icon={Factory}
          label="Pieces installed"
          value={s?.pieces_installed ?? "—"}
          to="/manufacturing"
        />
        <Kpi
          icon={AlertTriangle}
          label="Pieces damaged"
          value={s?.pieces_damaged ?? "—"}
          to="/manufacturing"
        />
      </div>
    </div>
