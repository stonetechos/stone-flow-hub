...
function NewSalesOrderPage() {
  ...
  const [form, setForm] = useState<SalesOrderCreateInput>({
    quote_id: null, project_id: null, customer_id: null,
    status: "draft", order_date: today(), delivery_date: null, notes: null,
  });
  ...
  return (
    <div>
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        ...
        <QuickForm.MoreDetails>
          ...
          <Field label="Source quote">
            <Select value={form.quote_id ?? ""} onValueChange={(v) => set("quote_id", v || null)}>
              ...
              <SelectContent>
                {(quotes.data ?? []).map((q) => (
                  <SelectItem key={q.id} value={q.id}>{q.quote_no}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </Field>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/sales-orders" })}>Cancel</Button>
