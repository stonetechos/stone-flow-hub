...
function EditSalesOrderPage() {
  ...
  useEffect(() => {
    if (query.data) {
      const r = query.data;
      setForm({
        quote_id: r.quote_id, project_id: r.project_id, customer_id: r.customer_id,
        status: r.status, order_date: r.order_date, delivery_date: r.delivery_date, notes: r.notes,
      });
    }
  }, [query.data]);
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        <QuickForm.QuickFill>
          <Field label="Customer">
            <Select value={form.customer_id ?? ""} onValueChange={(v) => set("customer_id", v || null)}>
        ...
        <QuickForm.MoreDetails>
          <Field label="Project">
            <Select value={form.project_id ?? ""} onValueChange={(v) => set("project_id", v || null)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field label="Source quote">
            <Select value={form.quote_id ?? ""} onValueChange={(v) => set("quote_id", v || null)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {(quotes.data ?? []).map((q) => <SelectItem key={q.id} value={q.id}>{q.quote_no}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Status">
            <Select value={form.status} onValueChange={(v) => set("status", v as SalesOrderCreateInput["status"])}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {SALES_ORDER_STATUSES.map((s) => <SelectItem key={s} value={s} className="capitalize">{s.replace(/_/g, " ")}</SelectItem>)}
              </SelectContent>
            </Select>
          </Field>
          <Field label="Delivery date">
            <Input type="date" value={form.delivery_date ?? ""} onChange={(e) => set("delivery_date", e.target.value || null)} />
          </Field>
        </QuickForm.MoreDetails>
        <QuickForm.Advanced>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/sales-orders/$id", params: { id } })}>Cancel</Button>
