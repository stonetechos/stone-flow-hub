    ...
    <div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <Input value={q} onChange={(e) => setQ(e.target.value)}
          placeholder="Search by SO no…" className="max-w-md" />
      ...
      {query.isLoading ? <LoadingBlock />
        ...
        : (query.data ?? []).length === 0 ? (
          <EmptyState
            ...
            action={<Button onClick={() => nav({ to: "/sales-orders/new" })}><Plus className="mr-2 h-4 w-4" /> New sales order</Button>}
          />
        ) : (
                ...
                {query.data!.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-mono text-xs">
                      <Link to="/sales-orders/$id" params={{ id: r.id }} className="text-primary hover:underline">{r.so_no}</Link>
                    </TableCell>
