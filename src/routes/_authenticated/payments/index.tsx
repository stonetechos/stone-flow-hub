...
function PaymentsPage() {
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success("Payment deleted"); qc.invalidateQueries({ queryKey: qk.paymentsAll.all }); setToDelete(null); },
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button onClick={() => nav({ to: "/payments/new" })}><Plus className="mr-2 h-4 w-4" /> New payment</Button>}
      />

      <div className="mb-3 flex items-center gap-2">
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search payment or reference…" className="max-w-md" />
      </div>
      ...
      {query.isLoading ? <LoadingBlock />
        ...
        : (query.data ?? []).length === 0 ? (
          <EmptyState icon={<Wallet className="h-6 w-6" />} title="No payments yet"
            message="Record a payment received against an invoice." action={<Button onClick={() => nav({ to: "/payments/new" })}><Plus className="mr-2 h-4 w-4" /> New payment</Button>} />
        ) : (
                ...
                {query.data!.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-mono text-xs">
                      <Link to="/payments/$id" params={{ id: r.id }} className="text-primary hover:underline">{r.payment_no}</Link>
                    </TableCell>
                    <TableCell className="font-mono text-xs">{r.invoice?.invoice_no ?? "—"}</TableCell>
                    <TableCell><Badge variant="outline" className="capitalize">{r.method.replace(/_/g, " ")}</Badge></TableCell>
      ...
      <ConfirmDialog open={!!toDelete} onOpenChange={(o) => !o && setToDelete(null)}
        title="Delete payment?" description={toDelete ? `${toDelete.payment_no} will be removed.` : ""}
