...
function EditPaymentPage() {
  ...
  useEffect(() => {
    if (query.data) {
      const r = query.data;
      setForm({
        invoice_id: r.invoice_id, amount: Number(r.amount), method: r.method,
        paid_at: r.paid_at.slice(0, 10), reference_no: r.reference_no, notes: r.notes,
      });
    }
  }, [query.data]);
  ...
  return (
    <div>
      ...
      <QuickForm onSubmit={(e) => { e.preventDefault(); mut.mutate(form); }} busy={mut.isPending}>
        ...
        <QuickForm.Actions>
          <Button type="button" variant="ghost" onClick={() => nav({ to: "/payments/$id", params: { id } })}>Cancel</Button>
