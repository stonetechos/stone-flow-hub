...
function InstallationDetailPage() {
  ...
  const query = useQuery({
    queryKey: qk.installations.byId(id),
    queryFn: () => getInstallation(id),
  });
  const progressQ = useQuery({
    queryKey: qk.installations.progress(id),
    queryFn: () => listProgress(id),
  });
  const materialsQ = useQuery({
    queryKey: qk.installations.materials(id),
    queryFn: () => listInstallationMaterials(id),
  });
  const signoffQ = useQuery({
    queryKey: qk.installations.signoff(id),
    queryFn: () => getSignoff(id),
  });
    ...
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="space-y-4 md:col-span-2">
            ...
            <CardContent className="text-sm">
              {!edit ? (
                <dl className="grid grid-cols-2 gap-y-1">
                  ...
                  <dd>{r.supervisor_name ?? "—"}</dd>
                  <dt className="text-muted-foreground">Site</dt>
                  <dd className="whitespace-pre-wrap">{r.site_address ?? "—"}</dd>
                  <dt className="text-muted-foreground">GPS</dt>
                  <dd>
                    {r.gps_lat != null && r.gps_lng != null ? `${r.gps_lat}, ${r.gps_lng}` : "—"}
                  </dd>
                  ...
                  <dd>{Number(r.progress_pct).toFixed(0)}%</dd>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-1">
                    <Label>Team</Label>
                    <Select
                      value={edit.team_id ?? "none"}
                      onValueChange={(v) => setEdit({ ...edit, team_id: v === "none" ? null : v })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      value={edit.supervisor_name}
                      onChange={(e) => setEdit({ ...edit, supervisor_name: e.target.value })}
                    />
                  </div>
                  <div className="md:col-span-2 space-y-1">
                    ...
                    <Textarea
                      rows={2}
                      value={edit.site_address}
                      onChange={(e) => setEdit({ ...edit, site_address: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      value={edit.gps_lat}
                      onChange={(e) => setEdit({ ...edit, gps_lat: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      value={edit.gps_lng}
                      onChange={(e) => setEdit({ ...edit, gps_lng: e.target.value })}
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEdit({ ...edit, planned_start_date: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEdit({ ...edit, planned_end_date: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      type="date"
                      value={edit.actual_start_date}
                      onChange={(e) => setEdit({ ...edit, actual_start_date: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      type="date"
                      value={edit.actual_end_date}
                      onChange={(e) => setEdit({ ...edit, actual_end_date: e.target.value })}
                    />
                  </div>
                  <div className="space-y-1">
                    <Label>Status</Label>
                    <Select
                      value={edit.status}
                      onValueChange={(v) =>
                        setEdit({ ...edit, status: v as InstallationOrderStatus })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Materials on site</CardTitle>
            </CardHeader>
            <CardContent className="text-sm">
                ...
                <ul className="divide-y">
                  {materialsQ.data.map((m) => (
                    <li key={m.id} className="grid grid-cols-6 gap-2 py-1.5 text-xs">
                      ...
                      <span>
                        Dm {m.qty_damaged} / Rt {m.qty_returned}
                      </span>
                    </li>
                  ))}
                </ul>
                    ...
                    <li key={p.id} className="py-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-muted-foreground">
                          ...
                          {p.labour_present ?? "—"} labour
                        </span>
                      </div>
                      {p.work_completed && <div className="text-xs">✓ {p.work_completed}</div>}
                      {p.balance_work && (
                        <div className="text-xs text-muted-foreground">
                          Balance: {p.balance_work}
                        </div>
                      )}
                      {p.material_shortage && (
                        <div className="text-xs text-destructive">
                          Shortage: {p.material_shortage}
                        </div>
                      )}
                      {p.customer_remarks && (
                        <div className="text-xs">Customer: {p.customer_remarks}</div>
                      )}
                      {p.supervisor_remarks && (
                        <div className="text-xs">Supervisor: {p.supervisor_remarks}</div>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <div className="space-y-4">
          {signoffQ.data && (
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Customer sign-off</CardTitle>
              </CardHeader>
              <CardContent className="text-sm">
                <p>
                  <span className="text-muted-foreground">By </span>
                  {signoffQ.data.customer_name ?? "—"}
                </p>
                <p>
                  <span className="text-muted-foreground">Rating </span>
                  {signoffQ.data.customer_rating ?? "—"}/5
                </p>
                <p>
                  <span className="text-muted-foreground">Signed </span>
                  {new Date(signoffQ.data.signed_at).toLocaleString()}
                </p>
                {signoffQ.data.remarks && (
                  <p className="mt-2 whitespace-pre-wrap">{signoffQ.data.remarks}</p>
                )}
              </CardContent>
            </Card>
          )}
