...
const GROUP_ORDER: ReadonlyArray<SearchGroupKey> = [
  "customers", "projects", "vendors", "products",
  "enquiries", "quotes", "salesOrders", "purchaseOrders",
  "inventory", "invoices", "payments", "dispatch",
];
...
export function GlobalSearchDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  return (
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      <CommandInput placeholder="Search customers, projects, enquiries, invoices…" value={query} onValueChange={setQuery} />
      <CommandList>
        {query.trim().length < 2 ? (
          <>
            {recent.length === 0 ? (
              ...
              <CommandGroup heading="Recent">
                {recent.slice(0, 8).map((r) => (
                  <CommandItem key={`${r.entityType}:${r.entityId}`} value={`recent-${r.label}`} onSelect={() => go(r.href)}>
                    <Clock className="mr-2 h-4 w-4 text-muted-foreground" />
                    <span className="truncate">{r.label}</span>
                    <span className="ml-auto text-xs text-muted-foreground capitalize">{r.entityType}</span>
                  </CommandItem>
                ))}
              </CommandGroup>
            )}
          ...
          <>
            {isFetching && <div className="px-3 py-2 text-xs text-muted-foreground">Searching…</div>}
              ...
              return (
                <div key={g}>
                  ...
                  <CommandGroup heading={items[0].groupLabel}>
                    {items.map((h) => (
                      <CommandItem key={`${g}:${h.id}`} value={`${g}-${h.label}-${h.id}`} onSelect={() => go(h.href)}>
                        <span className="truncate">{h.label}</span>
                        {h.sublabel && <span className="ml-auto truncate text-xs text-muted-foreground">{h.sublabel}</span>}
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </div>
