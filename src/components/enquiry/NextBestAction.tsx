    ...
    return (
      <Card className="shadow-1">
        ...
        <CardContent className="text-xs text-muted-foreground">
          All clear — no recommended action right now.
        </CardContent>
      </Card>
    );
  }
    ...
    <Card className="shadow-1">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="text-[11px] text-muted-foreground">
          Recommendations only. Nothing is executed automatically.
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className={cn("rounded-md border p-3 space-y-1.5", TONE[a.priority])}>
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="outline" className={cn("uppercase text-[10px]", TONE[a.priority])}>
          {a.priority}
        </Badge>
        <span className="text-sm font-semibold">{a.label}</span>
        {a.daysOverdue > 0 && (
          <span className="inline-flex items-center gap-1 text-[11px]">
            <Clock className="h-3 w-3" /> {a.daysOverdue}d overdue
          </span>
        )}
      </div>
      <div className="text-xs opacity-90">{a.reason}</div>
      <div className="text-[11px] opacity-80 flex items-center gap-1">
        <AlertCircle className="h-3 w-3" /> Expected: {a.expectedOutcome}
      </div>
      {a.href && (
        <Link
          to={a.href}
...
function RowAction({ a }: { a: NextBestAction }) {
  return (
    <div className="flex items-start justify-between gap-2 text-xs">
      ...
      <Badge variant="outline" className={cn("shrink-0 text-[10px] uppercase", TONE[a.priority])}>
        {a.priority}
      </Badge>
    </div>
  );
