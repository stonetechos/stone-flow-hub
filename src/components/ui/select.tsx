...
>(({ placeholder, ...props }, ref) => {
  ...
  const resolvedPlaceholder =
    typeof placeholder === "string" ? t(placeholder, placeholder) : placeholder;
  return <SelectPrimitive.Value ref={ref} placeholder={resolvedPlaceholder} {...props} />;
});
...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none focus:bg-[var(--intent-ghost-hover)] focus:text-foreground data-[state=checked]:font-medium data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        className,
      )}
      {...props}
    >
      ...
      <SelectPrimitive.ItemText>{renderedContent}</SelectPrimitive.ItemText>
    </SelectPrimitive.Item>
  );
});
