...
async function step(page, name, fn) {
  ...
  try {
    ...
    await page.screenshot({ path: join(OUT, `${results.length + 1}_${name.replace(/\W+/g, "_")}.png`) });
    results.push({ name, status: "PASS", ms });
    console.log(`✓ ${name} (${ms}ms)`);
  } catch (e) {
    const ms = Date.now() - started;
    await page.screenshot({ path: join(OUT, `${results.length + 1}_FAIL_${name.replace(/\W+/g, "_")}.png`) }).catch(() => {});
...
const listRoutes = [
  "/dashboard", "/customers", "/projects", "/enquiries", "/estimates",
  "/quotes", "/rfqs", "/purchase-orders", "/grns", "/manufacturing",
  "/inventory", "/dispatch", "/installations", "/invoices", "/receipts",
  "/payments", "/vendors", "/products", "/followups", "/tasks",
  "/documents", "/reports", "/notifications", "/dashboards/executive",
  "/dashboards/analytics", "/dashboards/procurement", "/dashboards/collections",
  "/dashboards/installation", "/dashboards/business-intelligence",
  "/dashboards/control-centre", "/dashboards/customer-intelligence",
  "/dashboards/vendor-intelligence", "/dashboards/forecast",
...
if (failCount) {
  console.log("\nFailures:");
  for (const r of results.filter((x) => x.status === "FAIL")) console.log(` - ${r.name}: ${r.error}`);
}
